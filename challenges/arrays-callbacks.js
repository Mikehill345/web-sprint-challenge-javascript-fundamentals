...
const displayNames = [];
zooAnimals.forEach(item => { return displayNames.push(`Name: ${item.animal_name}, Scientific: ${item.scientific_name}`) })
console.log(displayNames);



const lowCaseAnimalNames = zooAnimals.map(item => { return `${item.animal_name.toLowerCase()}` })
console.log(lowCaseAnimalNames);
...
let populationTotal = zooAnimals.reduce((acc, item) => { return acc + item.population }, 0)
